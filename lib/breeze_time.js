...
(function(){
    ...
    timeFormatRegEx = /^([1-9]|1[0-2])\:(0[0-9]|[1-5][0-9])[\s]?(AM|PM)/;
    ...
    BreezeTime.resolve = function(val){
        if(typeof val == 'number'){
            ...
            if(BreezeTime.isValidFormat(timeFormatRegEx)){
                return BreezeTime.parse(val);
            } else {
                return null;
            }
        }
    ...
    BreezeTime.prototype.asTime = function(){
        return [
            this.hour, ':', this.minute.toZeroPaddedString(2), this.period
        ].join('');
    };
